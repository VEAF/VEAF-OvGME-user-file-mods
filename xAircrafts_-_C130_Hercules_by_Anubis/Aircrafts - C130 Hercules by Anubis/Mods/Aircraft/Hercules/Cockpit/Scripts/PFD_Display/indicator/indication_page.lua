...
local function make_horizon_controllers_and_parameters(is_ladder)
	return {
		"PFD_roll",
		is_ladder and "PFD_ladder_pitch" or "PFD_horizon_pitch",
	}, {
		{"rotate_using_parameter",       0, 1.0},
		{"move_up_down_using_parameter", 1, -full_turn / 4, full_turn / 4},
	}
end
...
with_brightness(elem, {"PFD_bank"}, {{"rotate_using_parameter", 0, 1.0}})
elem.blend_mode     = blend_mode.IBM_REGULAR_RGB_ONLY
elem.parent_element = PFD_parent
AddElement(elem)
...
with_brightness(elem, {"PFD_heading_rad"}, {{"rotate_using_parameter", 0, 1.0}})
elem.blend_mode     	= blend_mode.IBM_REGULAR_RGB_ONLY
elem.parent_element 	= PFD_parent
AddElement(elem)
...
for i = 1, 4 do
	...
	local parameters, controllers = with_brightness(nil,
		{"PFD_alt_tape_y"}, {{"move_up_down_using_parameter", 0, 1.0 }}
	)
	Add_Thick_Line(
...
function rectangular_mask_horizontally_centered(name, width, height, x, y, go_down, level)
	write_log("rectangular_mask_horizontally_centered level = " .. level)
	local vertices = {
		{-width / 2, 0},
		{-width / 2, height},
		{ width / 2, height},
...
local function VVI_tower_shape(name_prefix, material, x_origin, y_origin, line_width, rectangle_width, rectangle_height, triangle_base, triangle_height, parameters, controllers, go_down, level)
	...
	-- write_log_json("vertices_indices", { vertices = vertices, indices = indices })
...
local parameters, controllers = with_brightness(nil, {"HudGS"}, {{"text_using_parameter", 0, 0}})
...
AddElement(elem)
--------------------------------------------------------------------------------------------------------------------------------------------HEADING DISPLAY
local parameters, controllers = with_brightness(nil, {"PFD_heading_deg"}, {{"text_using_parameter", 0, 0}})
...
with_brightness(elem, {"PFD_AGL"}, {{"parameter_in_range", 0, -1, 10000}})
elem.blend_mode     	= blend_mode.IBM_REGULAR_RGB_ONLY
elem.parent_element 	= PFD_parent
AddElement(elem)
...
with_brightness(elem, {"PFD_AGL"}, {{"parameter_in_range", 0, 10000, 50000}})
elem.blend_mode     	= blend_mode.IBM_REGULAR_RGB_ONLY
elem.parent_element 	= PFD_parent
AddElement(elem)

--------------------------------------------------------------------------------------------------------------------------------------------RADALT DISPLAY
local parameters, controllers = with_brightness(nil, {"PFD_AGL"}, {{"text_using_parameter", 0, 0}})
